...
d2 <- d2[d2$video_rank>0,]# & d2$video_rank<10000,]
d2 <- reshape2::melt(reshape2::acast(d2, system~task~team, fun.aggregate=min))
colnames(d2) <- c('system', 'task', 'team', 'video_rank')
d2 <- d2[is.finite(d2$video_rank),]
...
ggsave("plot-times.pdf", width=2, height=2, scale=1.5, units='in', 
  ggplot() +
  geom_violin(data=d[d$correctItem=='true',], trim=T, aes(x=system, y=time, fill=system, color=system), adjust=0.3, scale='count', size=.8) +
  facet_grid(.~taskType) +
